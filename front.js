...
function onload (fileNames) {
	for (let i = 0; i < fileNames.length; i++){
		let pokemonName = standardizeName(fileNames[i]);
	...
	for (let i = 0; i<pokemonList.length; i++){
		...
		if(pokemon === 'nidoran'){
			...
			box.appendChild(spriteDictionary['nidoranf']);
			box.appendChild(silhouetteDictionary['nidoranf']);
		}else{
