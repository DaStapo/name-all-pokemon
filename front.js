...
let paldeaList = ['Tauros-Paldea', 'Wooper-Paldea'];
...
let extraPokemon = {
    ...
    // Paldean forms in Paldeadex-order,
    'Annihilape' :['Wooper-Paldea'],
    'Grafaiai' :['Tauros-Paldea', 'Tauros-Paldea-Aqua', 'Tauros-Paldea-Blaze'],
    // Gen9 misc forms - maushold, squawkabilly, dudunsparce, tatsugiri, mascot ride; are just aesthetic
    'Oinkologne' :['Oinkologne-F'],
    'Palafin' :['Palafin-Hero'],
    'Sinistcha' :['Ursaluna-Blood-Moon'],
...
let gen8half = document.getElementById("pokemon-box-8-5")
...
for (key in formatted_lang_map){
    ...
    lang.onclick = function () {
		...
		for (let i = 0; i< formatted_lang_map[currentKey].length; i++){
            ...
            else if(pokemonList[i] === "urshifu"){
                unguessedDictTexts[standardizeName(pokemonList[i])].nodeValue = formatted_lang_map[currentKey][i] + " SS"
            }
            ...
            else if(pokemonList[i] === "ogerpon"){
                unguessedDictTexts[standardizeName(pokemonList[i])].nodeValue = formatted_lang_map[currentKey][i] + " T"
            }
            else{
			    unguessedDictTexts[standardizeName(pokemonList[i])].nodeValue = formatted_lang_map[currentKey][i];
            }
            ...
            for (let j  = 0; j<suffixes.length; j++){
                if (standardizeName(pokemonList[i])+suffixes[j] in  unguessedDictTexts){
                    ...
                    else if(suffixes[j] === "paldeaaqua"){
                        unguessedDictTexts[standardizeName(pokemonList[i])+'paldeaaqua'].nodeValue = formatted_lang_map[currentKey][i] + ' A';
                    }
                    ...
                    else if(suffixes[j] === "bloodmoon"){
                        unguessedDictTexts[standardizeName(pokemonList[i])+'bloodmoon'].nodeValue = formatted_lang_map[currentKey][i] + ' BM';
                    }
                    else if(suffixes[j] === "wellspring"){
                        unguessedDictTexts[standardizeName(pokemonList[i])+'wellspring'].nodeValue = formatted_lang_map[currentKey][i] + ' W';
                    }
                    else if(suffixes[j] === "hearthflame"){
                        unguessedDictTexts[standardizeName(pokemonList[i])+'hearthflame'].nodeValue = formatted_lang_map[currentKey][i] + ' H';
                    }
                    else if(suffixes[j] === "cornerstone"){
                        unguessedDictTexts[standardizeName(pokemonList[i])+'cornerstone'].nodeValue = formatted_lang_map[currentKey][i] + ' C';
                    }
                    ...
                    else if(suffixes[j] === 'rapidstrikegmax'){
...
function loadSprites() {
    ...
    for (let i = 0; i < paldeaList.length; i++) {
        let pokemon = standardizeName(paldeaList[i]);
        loadPkmn(pokemon);
    }
    ...
    for (let i = 0; i < fullSpriteList.length; i++) {
        ...
        if (pokemon == "meltan" || pokemon == "melmetal"){
            ...
            //there's 27 hisui
            if (current_index > index_8half && current_index < index_8half + 29){
...
let parseInput = function (inputText, sendLog, isTwitchChat) {
	...
	if (!document.getElementById("pokemon").disabled){
        ...
        if (inputText == "charizard" || inputText == "mewtwo"){
			...
			inputs.push(inputText + 'rapidstrike')
		}
...
let misspellings = {
    "Exeggcute":["eggsecute", "egsecute", "eggsacute","eggsucute", "exacute","eggzercute","exegut","exxute"],
    "Exeggutor":["execetue","eggcecutor","eggexctor","eggsakute","eggsecutor","eggsegitor"],
    "Victreebel":["victorybell", "victrible"],
    "Phanpy":["fanfie"],
    "Bulbasaur":["balbasoa"],
    "Abra":["arba"],
    "Ho-oh":["hoho"],
    "Munna":["mauna","moona"],
    "Arceus":["arcues","arsues","arsuse"],
    "Gyarados":["gryados","gaiados","garyados","garidos","garyados","garidoes","garidose", "garaodos","gardos"],
    "Barraskewda":["barroskuda","barrakuda","barakuda","barracuda","baracuda"],
    "Azelf":["aself", "axelf"],
    "Wobbuffet":["wabafet","wabafett","wabafett","wabafette","wubafett","wubafet","wobofett","wobafett","wabofet","wobafet","woobafett","wubbifat","wubbfat","wabbofette"],
    "wynaut":["whynot"],
    "Zubat":["zoobat"],
    "Marowak":["maroark"],
    "Zeraora":["zaroara","zaroaro"],
    "Golbat":["globat"],
    "Amoonguss":["amongoos"],
    "Foongus":["fungoos"],
    "Eiscue":["icecue","iceque","icesque"],
    "Beheeyem":["beeheam","beheam","behymm"],
    "Banette":["bannet","bennet"],
    "Goldeen":["goldine","goldin"],
    "Bellossom":["belosom"],
    "Bronzor":["bronzon"],
    "Buneary":["bunnery","bunnerry"],
    "Carnivine":["carvine"],
    "Wartortle":["waturtol","wortle"],
    "Kakuna":["cacoona","cocoona"],
    "Chelterrar":["cheltrain"],
    "Chimecho":["chimcheo"],
    "Chinchou":["chinchaw"],
    "Lanturn":["luntern"],
    "Incineroar":["cinderoar", "incereor"],
    "Cosmoem":["cosmeon"],
    "Anorith":["amoranth", "armoranth","arinoth"],
    "Blastoise":["blasidos", "blastios", "blasdios", "blastious"],
    "Kubfu":["cubfoo"],
    "Ursaluna":["ursula"],
    "Beautifly":["buityfly","butterrfly","butifly"],
    "Darkrai":["darkria","drakria"],
    "Ducklett":["duckling"],
    "Pelipper":["peppiler", "pellipur"],
    "Pawniard":["pewnidar"],
    "Kyurem":["kyrume"],
    "Leavanny":["livany"],
    "Drowzee":["drawsie","drowesy","drousie","drowsy","drowsi"],
    "Chansey":["chancie","chancy","chauncy","chency"],
    "Coalossal":["closacoal","closalcoal","clossalcoal","colasalkol","collasalkoal"],
    "Druddigon":["duddrangion","dreadrugan","dudragion","dredigan", "dudraggion","duddreigon","dudregon"],
    "Drifloon":["drifbloom"],
    "Purrloin":["purlion","perlion"],
    "Lumineon":["lumeion", "lumioir", "lumin"],
    "Regieleki":["regilektrik"],
    "Mimikyu":["mimicue","mimkue"],
    "Eelektrik":["eelectrike"],
    "Alomomola":["amola","ammola"],
    "Aurorus":["amorous", "amourus", "amoreus", "amorus"],
    "Tentacool":["tenteqill","tentakul","tentcul"],
    "Serperior":["supuriur","serpioper"],
    "Floette":["flaoute"],
    "Gossifleur":["gosiflower","gosiflor"],
    "Sigilyph":["siglith","syglaith","sigylith","syglith","syglithe",],
    "Diglett":["digglit"],
    "Skarmory":["skamrow"],
    "Zoroark":["zorurk"],
    "Froakie":["frokey","frooggy"],
    "Aromatisse":["armoteze"],
    "Aerodactyl":["arodatle","areodacktly"],
    "Grovyle":["grovly","groyvle","growl","growli"],
    "Jangmo-o":["jagamoo","jammoo","yangomo"],
    "Hatterene":["hatrenne", "hatturn"],
    "Hattrem":["hattern",],
    "Hatenna":["hattene"],
    "Gigalith":["goliolith"],
    "Cherubi":["cherrbery"],
    ...
    "Boldore":["bouldour"],
    "Ninetales":["nintails"],
    "Wigglytuff":["wigglybuff"],
    "Sylveon":["syveleon"],
    "Carvanha":["caravna"],
    "Kicklee":["kikli"],
    "Klink":["cling", "kling"],
    "Inkay":["inkling"],
    "Hakamo-o":["hockmoo", "hakommoo"],
    "Vullaby":["vlabby"],
    "Kricketot":["cricket"],
    "Kricketune":["kricketoon","krickitoon"],
    "Kyogre":["keyoreg","kyoreg"],
    "Tornadus":["windorus","windrus"],
    "Ledyba":["laydeeba"],
    "Lairon":["larion"],
    "Pineco":["pincone"],
    "Electrike":["eletrick"],
    "Lunala":["loona"],
    "Decidueye":["desudguai","dicidui","dicidia"],
    "Lunatone":["lunasto"],
    "Magmar":["magamar","margamar","magnamar"],
    "Omanyte":["omanight","omynight"],
    "Reuniclus":["reneculois"],
    "Mantine":["manatee"],
    "Octillery":["octliri"],
    "Manectric":["manectike","manektike","manetrike"],
    "Corviknight":["corvanite","corvankite"],
    "Damythir":["damhintyr"],
    "Beldum":["belldang"],
    "Mienfoo":["minphoo", "meinphew","meinfeew","meinfew","minfew","minfeew"],
    "Mienshao":["miancham"],
    "Mime Jr.":["mimejunior","jrmime"],
    "Forretress":["fortree"],
    "Mareanie":["meriane","marrine","meraini"],
    "Magnemite":["magnyte"],
    "Jynx":["jnix","jnyx"],
    "Spritzee":["spreetse"],
    "Politoed":["pollitoad"],
    "Pinsir":["pincher","pinsour","pinseer","pintcer"],
    "Smoochum":["smucham"],
    "Tangrowth":["tangtrow","tangthrow"],
    "Flaaffy":["phlaffy"],
    "Finneon":["phineon"],
    "Pikachu":["piakchu","pikchu"],
    "Raichu":["richu","richue","riachu","rychu","riyachu"],
    "Pidgeotto":["pidgeoto","pidgyoto","pigioto"],
    "Pidgey":["pigie","pidgie"],
    "Pachirisu":["parsachu","parsachi","parsachai","parsachu","parsasch","parsascha","parsaschi","parschachu","parschasu","piraschu"],
    "Typhlosion":["tiflosione"],
    "Garbodor":["trabodor"],
    "Rattata":["ratta"],
    "Krookodile":["krookidle"],
    "Golett":["gollet"],
    "Graveler":["growler"],
    "Growlithe":["growleth"],
    "Squirtle":["squtile","squrlete","skurtel","skwurtel","swortle","Squeital","sqortl"],
    "Salandit":["sandalitt"],
    "Yungoos":["yangoose"],
    "Sizzlipede":["slispede"],
    "Weezing":["weasing"],
    ...
    "Weedle":["weedol"],
    "Seviper":["servipre"],
    "Suicune":["suicoon","suiqoon"],
    "Scizor":["skissors"],
    "Toxtricity":["toxisitree","toxicitree"],
    "Thwackey":["twaky"],
    "Buizel":["buweasle","bweasle","beuzal"],
    "Sneasel":["sneals","snezle","sneasle","sneazle","sneezle","sneesle","snevel","snevell"],
    "Snubbull":["snubble","snuble"],
    "Tangela":["tangler"],
    "Staravia":["staravior"],
    "Staryu":["starey","stary"],
    "Starmie":["starmy","starmey"],
    "Stonjourner":["stonehenge","stonhenge"],
    "Tsareena":["tserenee"],
    "Beedrill":["bidril"],
    ...
    "Tauros":["toros","torus","torous"],
    "Torracat":["torkkat","toraket"],
    "Sobble":["sobel"],
    "Toxicroak":["toxekro"],
...
function updateTypeFilter(type){
    ...
    regionToSingle(document.getElementById("region8-5"));
...
function updateGenFilter() {
    ...
    if (currentGen === 0) {
        ...
        regionToAll(document.getElementById("region8-5"));
        document.getElementById("pokemon-box-7-5").classList.remove('unknownbox');
...
let spriteCycles = {
    ...
    "maushold":["maushold","maushold-of-three"],
    ...
    "ogerpon":["ogerpon","ogerpon-teal-mask"],
